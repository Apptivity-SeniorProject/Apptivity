  ...
  const handleSuggestEventsPress = async () => {
    ...
    try {
      ...
      if (result.status === 'depleted') {
        toast.info(
          result.message ?? 'Bugunluk buralardaki tum paslari tukettin kral. Yeni etkinlikler eklendiginde ilk senin haberin olacak!'
        );
        return;
      }

      toast.error(result.message ?? 'Su anda oneri servisi kullanilamiyor. Lutfen daha sonra tekrar dene.');
    } catch {
